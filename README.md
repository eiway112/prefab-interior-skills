...
2. 将 `技能仓备份/` 下所需的技能目录复制到 Qoder 技能目录（旧版 QoderWork 与本版共用同一运行时目录）：
   - Windows：`C:\Users\<你的用户名>\.qoder\skills\`
   - macOS / Linux：`~/.qoder/skills/`
...
| 目录 / 文件 | 说明 |
|---|---|
| `技能仓备份/` | **对外引用入口**：15 个技能本体 + 治理文件完整镜像，可直接复制使用 |
| `standards-index.md`（技能仓备份根） | 规范标准索引（69 条现行标准登记与核验记录） |
...
`waterproofing-expert`（建筑防水工程技术顾问）已于 2026-08-14 迁出本合集，独立维护于建筑专业辅材技能合集项目（见 CG-20260814-003）；本合集各技能保留对其的跨合集外部协同引用。
