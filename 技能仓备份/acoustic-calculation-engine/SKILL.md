...
Step 2: 根据评价量选择计算模型组合
         ...
         撞击声隔声 → M5 + M6
         提供有效基准数据时 → M7 校准优先，M1-M6 交叉核对（见 §七）
...
对称变体取区间中值，输出区间；单侧变化按该侧比值取 20·lg
...
M7 结果须与 M1-M6 全模型估算交叉核对。两者差 > 5 dB 时，优先报 M7 校准值（数据锚点可信度更高），同时保留 M1-M6 值并说明差异来源（通常为 M1-M6 低估空腔增益、声桥取值偏悲观）。
